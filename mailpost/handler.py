        ...
        for msg_rule in self.mappings:
            ...
            for key, pattern in rule['conditions'].items():
                ...
                if type(pattern) in [list, tuple]:
                    ...
                    raise ConfigurationError("Pattern should be string or list, not %s" %\
                                             type(pattern))
            if match:
                return url, rule
        return None
        ...
        for message in inbox:
            ...
            if options['send_files']:
                for num, attachment in enumerate(message.attachments):
                    ...
                    file_param = MultipartParam('attachment[%d]' % num, filename=filename, filetype=ctype, fileobj = fileobj)
            ...
            for name in options['msg_params']:
                ...
                if part: #TODO: maybe we should raise an exception if there's no part
                    data[name] = part
